...
# -----------------------------------------
# 3. CARICAMENTO DATI (TRAMITE UPLOAD)
# -----------------------------------------
@st.cache_data
def load_data(file):
    try:
        df_ordini = pd.read_excel(file, sheet_name='Ordini', engine='openpyxl')
        df_prodotti = pd.read_excel(file, sheet_name='Prodotti Magazzino', engine='openpyxl')
...
if check_password():
    ...
    with st.sidebar:
        ...
        uploaded_file = st.file_uploader("📂 Carica il Gestionale (Excel)", type=["xlsx"])
        
        st.markdown("---")
        
        # I filtri appaiono solo se il file è caricato
        if uploaded_file is not None:
            df_ordini, df_prodotti = load_data(uploaded_file)
    ...
    if uploaded_file is None:
        st.info("👈 Per iniziare, carica il tuo file Excel dalla barra laterale sinistra.")
    else:
        st.markdown("---")
